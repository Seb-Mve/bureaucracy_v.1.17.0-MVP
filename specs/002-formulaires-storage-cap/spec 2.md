...
**Created**: 2025-01-24  
**Status**: Draft  
**Input**: User description: "Créer une barrière administrative invisible et irrationnelle pour forcer la progression via une limite de stockage des formulaires avec déblocages progressifs"

## Clarifications

### Session 2025-01-24

- Q: Où exactement le compteur rouge clignotant apparaît-il sur l'écran ? → A: Option B — Le compteur rouge clignotant apparaît dans la section ressources en haut de l'écran principal (même zone que les autres ressources).
- Q: Comment la production automatique interagit-elle avec l'achat d'amélioration de stockage ? → A: Option C — L'achat est atomique : stock remis à 0 d'abord, puis la production automatique reprend normalement dans la nouvelle limite.
- Q: Faut-il inclure les mécanismes de prestige (Extension des Classeurs, Trombones, Réforme Administrative, Valeur Administrative) dans cette feature ? → A: Non. On ignore toute notion de prestige pour le moment. La feature se concentre uniquement sur le mécanisme de blocage et les 4 upgrades séquentiels.
- Q: Quand les améliorations de stockage deviennent-elles visibles dans les menus d'Administration ? → A: Option A — Visibilité conditionnelle : les upgrades n'apparaissent dans les menus d'administration que lorsque le compteur est bloqué (rouge clignotant). Pas d'indice avant.
- Q: Quand le clignotement rouge s'arrête-t-il après l'achat d'une amélioration ? → A: Option A — Le clignotement rouge s'arrête immédiatement dès que le stock repasse à 0 après l'achat. Retour couleur normale instantané.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Découverte du Blocage Initial (Priority: P1)

Le joueur produit des formulaires normalement jusqu'à atteindre le premier seuil de stockage. Au moment où le stock atteint exactement 983 formulaires, le compteur (situé dans la section ressources en haut de l'écran principal) se fige, le nombre passe en rouge (#FF0000) et commence à clignoter. Aucune explication n'est fournie, créant une tension et incitant le joueur à explorer les menus d'Administration pour comprendre ce qui se passe.
...
4. **Given** le joueur achète une amélioration de stockage avec production automatique active, **When** l'achat est confirmé, **Then** le stock passe à 0 de manière atomique et la production automatique reprend immédiatement dans la nouvelle limite sans perte de formulaires
...
### User Story 3 - Gestion du Surplus Automatique (Priority: P3)
...
- **Que se passe-t-il si le joueur achète une amélioration de stockage pendant qu'une production automatique est active ?** L'achat est une transaction atomique : le stock est remis à 0 en premier, puis la nouvelle limite est appliquée, puis la production automatique reprend normalement sans perte de formulaires générés pendant la transaction.
...
- **FR-001**: Le système DOIT bloquer le compteur de formulaires lorsque le stock atteint exactement 983 formulaires
...
- **FR-005**: Le système DOIT rendre visibles les améliorations de stockage dans les menus d'Administration uniquement lorsque le compteur est bloqué (clignotement rouge actif) ; les améliorations NE DOIVENT PAS être visibles avant que la limite ne soit atteinte pour la première fois
...
- **FR-008**: Le système DOIT remettre le stock de formulaires à zéro immédiatement après l'achat d'une amélioration de stockage (opération atomique : reset avant reprise de production)
- **FR-009**: Le système DOIT appliquer la nouvelle limite immédiatement après l'achat d'une amélioration
- **FR-009.1**: Le système DOIT garantir que la production automatique reprend normalement dans la nouvelle limite après l'achat d'une amélioration (aucune perte de formulaires produits pendant la transaction)
...
- **FR-011**: Le système DOIT rejeter tout formulaire produit lorsque le stock est au plafond (surplus définitivement perdu)
- **FR-012**: Le système DOIT figer le compteur strictement au plafond même si une production automatique tente de dépasser cette limite
- **FR-013**: Le système NE DOIT PAS afficher de notification lorsque des formulaires produits automatiquement sont perdus au plafond
- **FR-014**: Le système DOIT afficher le compteur en couleur normale (non-rouge, non-clignotant) lorsque le stock est inférieur à la limite
- **FR-015**: Le système DOIT arrêter immédiatement le clignotement rouge et restaurer la couleur normale du compteur dès que le stock repasse à 0 après l'achat d'une amélioration de stockage (comportement instantané synchronisé avec le reset du stock)
...
- **Limite de Stockage (Storage Cap)**: Valeur numérique représentant le plafond actuel de formulaires. Peut être 983, 1 983, 4 583, 11 025, ou null (illimité)
...
- **SC-004**: Le compteur se bloque exactement à la valeur du plafond (983, 1 983, 4 583, 11 025) sans jamais dépasser ni sous-estimer d'une unité
...
- **SC-006**: Aucun formulaire ne peut être ajouté au stock lorsque la limite est atteinte, garantissant une application stricte du plafond
- **SC-007**: Le clignotement du compteur rouge reste stable à une fréquence de 2 Hz (±0.5 Hz) pour une visibilité optimale sans risque épileptique
